      ...
      (with-transformed-position (tr x1 y1)
	      (with-transformed-position (tr x2 y2)
	        (slet ((p1 (ns-make-point (pixel-center x1)
	          ...
	          #+nil(send mirror :stroke-path path :in-colour colour)))))))
      ...
      (with-transformed-position (tr right top)
        (with-beagle-graphics (medium)
          ...
          (when (and filled (or (typep ink 'climi::transformed-design)
            ...
            (send mirror
                  :draw-image colour
                  :at-point (ns-make-point (pixel-center left)
                                           (pixel-center top)))
...
(defmethod medium-draw-text* ((medium beagle-medium) string x y
			                        ...
			                        toward-x toward-y
                              transform-glyphs)
  (declare (ignore toward-x toward-y transform-glyphs transformation))
